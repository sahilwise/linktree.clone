...
<div className="relative max-w-2xl mx-auto py-3" >
        <div className="flex justify-between items-center text-sm sm:mx-5 mx-6">
            ...
            <span onClick={()=>{router.push('mailto:hi@salnet.xyz')}} className="whitespace-nowrap inline-block group-hover:max-w-screen-2xl group-focus:max-w-screen-2xl max-w-0 scale-80 group-hover:scale-100 overflow-hidden transition-all duration-500 group-hover:px-2 group-focus:px-2">Contact</span>
            </span>
            </button>
        ...
        <div className="grid grid-cols-3 shadow-sm text-xs gap-x-2 bg-gray-50 border border-black border-b-2 border-l-2 py-1.5 rounded-3xl mx-4">
            <button style={{cursor: 'url(handler.png), auto'}} onClick={()=>{router.push('/')}} className="mx-auto bg-black text-white px-5 py-1.5 rounded-3xl">
                Links
            </button>
            ...
            <button style={{cursor: 'url(handler.png), auto'}} onClick={()=>{router.push('/store')}} className="mx-auto text-black px-5 py-1.5 rounded-3xl">
            Store
            </button>
        </div>

        <div className="grid gap-0.5 mt-5 mx-5 ">
            <LinkCard  Imager="addify" Title="Create your addify id" domain="addify.us" />
            <LinkCard Imager="ytube" Title="Create your ytube link" domain="ytube.click" />
            <LinkCard Imager="wapp" Title="Create your wapp link" domain="wapp.ink" />
            <LinkCard Imager="o" Title="Appmojo" domain="appmojo.site" />
